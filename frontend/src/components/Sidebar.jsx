...
// Renders the hamburger; place it inside the topbar. Hidden on desktop via CSS.
export function MenuButton({onClick}){
  return <button className="menu-toggle" onClick={onClick} aria-label="Abrir menú"><MenuIcon/></button>
}
...
export default function Sidebar({open,onClose}){
  ...
  useEffect(()=>{onClose&&onClose()},[location.pathname])
  const go=p=>{navigate(p);onClose&&onClose()}
  return(<>
    ...
    <aside className={`sidebar ${open?'open':''}`}>
      ...
      {[['/','Inicio',<HomeIcon/>],['/calendario','Calendario',<CalIcon/>],['/pendientes','Pendientes',<PendIcon/>]].map(([path,label,icon])=>(
        <button key={path} className={`sidebar-item ${a(path)?'active':''}`} onClick={()=>go(path)}>{icon}<span>{label}</span></button>
      ))}
      <div className="sidebar-spacer"/>
      <button className={`sidebar-item ${a('/ajustes')?'active':''}`} onClick={()=>go('/ajustes')}><CfgIcon/><span>Ajustes</span></button>
