...
const{id}=useParams();const navigate=useNavigate();const location=useLocation()
  ...
  useEffect(()=>{Promise.all([api.getClass(id),api.getStudents(id),api.getPeriods(id),api.getModels(id)]).then(([c,s,p,m])=>{setCls(c);setStudents(s);setPeriods(p);setModels(m)}).catch(console.error).finally(()=>setLoading(false))},[id])
